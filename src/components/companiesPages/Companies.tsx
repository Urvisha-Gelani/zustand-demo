          ...
          <div className='flex flex-wrap justify-between items-center '>
            ...
            <div className={`w-[10%] ${(allCompanies.length === 0) ? "block" : "block"}`}>
              <Popup trigger={<button className='px-[10px] py-[9px] w-full bg-[#304463] text-white rounded-[8px] flex items-center justify-center gap-[8px] text-[15px]'>
                ...
                {(close) => (
                  <Company close={close} title="Add Company" page={currentPage} />
                )}
              </Popup>
            </div>
          </div>
                ...
                (compnayLoading) ? <div className=' absolute position-ab opacity-[1] z-[3]'>
                  ...
                  <div className='h-[531px] overflow-y-auto scrollbar-custom mt-[20px]'>
                    {(allCompanies.length === 0) ? <div className='text-center  text-gray-400'><p>No Companies Found</p></div> : <table className="w-[70vw] px-[10px] mx-auto text-center table-striped border mt-[15px]">
                      <thead className=''>
                        <tr className=''>
                          <th className="border-b-2 border-gray-300 border-2 px-[5px] py-[15px]">Name</th>
                          <th className="border-b-2 border-gray-300 border-2 px-[5px] py-[15px]">Type</th>
                          <th className="border-b-2 border-gray-300 border-2 px-[5px] py-[15px]">Location</th>
                          <th className="border-b-2 border-gray-300 border-2 px-[5px] py-[15px]">About</th>
                          <th className="border-b-2 border-gray-300 border-2 px-[5px] py-[15px]">Created</th>
                          <th className="border-b-2 border-gray-300 border-2 px-[5px] py-[15px]">Updated</th>
                          <th className="border-b-2 border-gray-300 border-2 px-[5px] py-[15px] sticky top-0 right-[-9px]  bg-white ">Action</th>
                        </tr>
                      </thead>
                      <tbody>
                        {
                          ((allCompanies as companyType).map((company: companyType) => {
                            return (<tr key={company.id} className='py-[15px]'>
                              <td className="border-b border-gray-300 border-2  px-px-[5px] py-[15px]">{company.name}</td>
                              <td className="border-b border-gray-300 border-2  px-px-[5px] py-[15px]">{company.type}</td>
                              <td className="border-b border-gray-300 border-2  px-px-[5px] py-[15px]">{company.location}</td>
                              <td className="border-b border-gray-300 border-2  px-px-[5px] py-[15px]">{company.about}</td>
